...
export const SyncModal: React.FC<SyncModalProps> = ({
  ...
  return (
    ...
    >
      <div className="w-full max-w-xl bg-[#12131a] border border-white/[0.1] rounded-2xl p-6 sm:p-7 shadow-2xl relative my-auto">
        {/* Header */}
            ...
            <div
              ...
              className={`border border-dashed rounded-xl p-7 text-center cursor-pointer transition-all duration-150 relative ${
                dragActive
                  ...
                  : 'border-white/[0.12] hover:border-white/[0.25] bg-[#0b0c10]/80'
              }`}
            >
              <input
                ref={fileInputRef}
              ...
              <div className="w-12 h-12 mx-auto mb-3 rounded-xl bg-zinc-800 text-zinc-300 flex items-center justify-center border border-white/[0.06]">
                {isProcessing ? (
                  <RefreshCw className="w-6 h-6 text-indigo-400 animate-spin" strokeWidth={1.5} />
                ) : (
              ...
              <div className="mt-4 flex items-center justify-center gap-2 flex-wrap text-[11px] font-mono text-zinc-400">
                <span className="px-2 py-0.5 rounded bg-zinc-800 border border-white/[0.06] flex items-center gap-1">
                  <FolderArchive className="w-3 h-3 text-indigo-400" /> .zip (Vault)
                </span>
                <span className="px-2 py-0.5 rounded bg-zinc-800 border border-white/[0.06] flex items-center gap-1">
                  <FileText className="w-3 h-3 text-indigo-400" /> .md (Playbook)
                </span>
              </div>
            </div>
        ...
        {inspection && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2.5">
              <div className="p-3 rounded-xl bg-[#0b0c10]/80 border border-white/[0.06] text-center">
            ...
            {inspection.domains.length > 0 && (
              <div className="p-3.5 rounded-xl bg-[#0b0c10]/80 border border-white/[0.06] space-y-2.5">
                <div>
                  <div className="flex items-center justify-between gap-1.5 text-xs font-semibold text-zinc-300 mb-1.5">
                    <div className="flex items-center gap-1.5">
                      <Layers className="w-3.5 h-3.5 text-indigo-400" strokeWidth={1.5} />
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    {(showAllDomains ? inspection.domains : inspection.domains.slice(0, 4)).map((d) => (
                      ...
                      >
                        <span>{d.name}</span>
                        <span className="font-mono text-indigo-400 text-[10px]">({d.count})</span>
                      </span>
                    ))}
                    {!showAllDomains && inspection.domains.length > 4 && (
                      <button
                        ...
                        className="text-[10px] font-mono px-2 py-0.5 rounded-md bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 border border-indigo-500/20 transition-colors"
                      >
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {!showAllSubdomains && inspection.subdomains.length > 4 && (
                        <button
                          ...
                          className="text-[10px] font-mono px-2 py-0.5 rounded bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 border border-indigo-500/20 transition-colors"
                        >
            ...
            {inspection.tools.length > 0 && (
              <div className="p-3 rounded-xl bg-[#0b0c10]/80 border border-white/[0.06]">
                <div className="flex items-center gap-1.5 text-xs font-semibold text-zinc-300 mb-2">
                  <Wrench className="w-3.5 h-3.5 text-indigo-400" strokeWidth={1.5} />
                ...
                <div className="flex items-center gap-1 flex-wrap font-mono text-[10px] text-zinc-400">
                  ...
                  {!showAllTools && inspection.tools.length > 6 && (
                    <button
                      ...
                      className="px-2 py-0.5 rounded bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 border border-indigo-500/20 transition-colors"
                    >
