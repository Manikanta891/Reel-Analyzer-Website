...
export const TopBannerAdSpot: React.FC<TopBannerAdSpotProps> = ({
  ...
  const devMockup = (
    <div className="mb-6 rounded-xl bg-[#12131a] border border-white/[0.08] p-2.5 sm:px-4 flex items-center justify-between gap-3 text-xs">
      <div className="flex items-center gap-2 flex-wrap min-w-0">
        <span className="text-[10px] uppercase font-semibold tracking-wider px-2 py-0.5 rounded bg-zinc-800 text-zinc-400 border border-white/[0.06] flex items-center gap-1">
          <Sparkles className="w-2.5 h-2.5 text-indigo-400" strokeWidth={1.5} />
          <span>Partner</span>
        </span>
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 hidden md:inline">
          DEV PREVIEW
        </span>
        <a
          href={link}
