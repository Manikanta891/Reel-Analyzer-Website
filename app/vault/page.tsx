...
export default function VaultPage() {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [selectedDomain, selectedSubdomain, selectedCreator, searchQuery, feedViewMode, activeTab]);
  ...
  // Compute Top Creators (excluding 'unknown', 'n/a', empty, etc.)
  const creatorsData = useMemo(() => {
    const counts: { [creator: string]: number } = {};
    reels.forEach((r) => {
      const raw = r.author?.trim();
      if (!raw) return;
      const lower = raw.toLowerCase();
      // Strict exclusion of unknown and non-creator identifiers
      if (
        [
          'unknown',
          'n/a',
  ...
  // Regex-safe Search & Filtering (incorporating Category, Subcategory, and Creator)
  const filteredReels = useMemo(() => {
    return reels.filter((r) => {
      ...
      if (selectedCreator !== 'All') {
        const raw = (r.author || '').trim();
        const clean = raw.startsWith('@') ? raw.slice(1).trim() : raw;
        creatorMatch = clean.toLowerCase() === selectedCreator.toLowerCase();
      }

      if (!domainMatch || !subMatch || !creatorMatch) return false;
  ...
  }, [reels, selectedDomain, selectedSubdomain, selectedCreator, searchQuery]);
                ...
                <div>
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 mb-4">
                    ...
                    <div className="relative flex-1">
                      ...
                      <input
                        type="text"
                        placeholder="Search by topic, tools (Docker, Tailwind), creator, or keyword..."
                        value={searchQuery}
                    ...
                    {/* Right controls: Active Creator Badge + Filter Counter + View Mode Toggle */}
                    <div className="flex items-center gap-2 flex-shrink-0 flex-wrap sm:flex-nowrap">
                      {selectedCreator !== 'All' && (
                        <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-brand-600/20 border border-brand-500/30 text-xs text-brand-300 font-medium">
                          <Users className="w-3.5 h-3.5 text-brand-400" />
                          <span>@{selectedCreator}</span>
                          <button
                            onClick={() => setSelectedCreator('All')}
                            className="text-brand-400 hover:text-white ml-0.5 p-0.5 rounded hover:bg-brand-500/20 transition-colors"
